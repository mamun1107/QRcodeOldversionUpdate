...
extension WAVC {
    ...
    @objc func createBtnTapped(_ sender: UIButton) {
           print("Created Whats App")
        
        if let qrtitle = titleTextField.text  {
            if !qrtitle.isEmpty && qrtitle != "" && !phoneNumField.text!.isEmpty && phoneNumField.text != "" {

                guard let whatsAppNumber = phoneNumField.text else {return}
                //guard let name = nameTextField.text else {return}
             
                self.gScanDataArray.insert(GScanObject(qrTitle: qrtitle, qrString: whatsAppNumber, qrTime: getNowTime(), others:"" ,isSelected: false), at: 0)
                
                self.saveToUserDefaults()
                let storyboard = UIStoryboard (name: "Main", bundle: nil)
                if let vc = storyboard.instantiateViewController(withIdentifier: "DetailVC") as? DetailVC {
                    vc.qrTime = getNowTime()
                    vc.qrString = "Whats App Number is: \(whatsAppNumber)"
